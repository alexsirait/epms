...
class IT extends CI_Controller
{
	...
	public function hrd_it()
	{
		...
		$data['data_epms'] = $this->dashboard_model->tampilhrd_it();

        //per section
		$data['sect_it'] = $this->dashboard_model->sect_it();
        $data['sect_it_epicor'] = $this->dashboard_model->sect_it_epicor();
        $data['sect_epicor'] = $this->dashboard_model->sect_epicor();
		...
		$this->load->view('templates/dashboard', $data);
		$this->load->view('templates/topbar', $data);
		$this->load->view('home/hrd_it', $data);
		$this->load->view('templates/footer');
		$this->load->view('templates/footer_login');
	}
	...
	{
		$this->load->model('dashboard_model');
		$data['judul'] = 'Data PMS Dept. IT & EPICOR ( Section : IT )';
		...
		$data['data_epms'] = $this->dashboard_model->it_it();
		...
		$this->load->view('templates/dashboard', $data);
		$this->load->view('templates/topbar', $data);
		$this->load->view('epms/manage-it', $data);
		$this->load->view('templates/footer');
		$this->load->view('templates/footer_login');
	}

    public function it_epicor()
	{
		$this->load->model('dashboard_model');
		$data['judul'] = 'Data PMS Dept. IT & EPICOR ( Section : IT & EPICOR )';
		...
		$data['data_epms'] = $this->dashboard_model->it_it_epicor();
		...
		$this->load->view('templates/dashboard', $data);
		$this->load->view('templates/topbar', $data);
		$this->load->view('epms/manage-it_epicor', $data);
		$this->load->view('templates/footer');
		$this->load->view('templates/footer_login');
	...
	{
		$this->load->model('dashboard_model');
		$data['judul'] = 'Data PMS Dept. IT & EPICOR ( Section : EPICOR )';
		...
		$data['data_epms'] = $this->dashboard_model->it_epicor();
    ...
    function emailmanagerlv3(){
		$loginlink =  site_url() . 'login' . '"';
			$fullname = 'Craig Duncan';
			$department = 'IT & EPICOR';
			...
			$data = array(
				...
				'content'	=> $textContent,
				'email' 	=> 'satoto.subandoro@icloud.com',
                //'email' 	=> 'Craig.duncan@cladtek.com',
				'nama' 		=> 'Craig Duncan',
				'department' => 'IT & EPICOR',
