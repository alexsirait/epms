...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Performance Management System Report</h6>
    </div>
    <div class="card-body">
    <form id="form1" action="<?php echo base_url('gatepass/CheckDelete1') ?>" method="POST"> 
  <!-- <button type="button"   class=" m-3 mb-3 ml-3 btn btn-danger" onclick="deletedata()"><i class="fas fa-fw fa-trash-alt"></i> Delete</button> -->
    ...
    <!-- <form id="form1" action="<?php echo base_url('gatepass/CheckDelete1') ?>" method="POST"> -->
            <table class="table table-hover table-bordered" id="dataTable" width="100%" cellspacing="0">
                <thead class="thead-light">
                    <tr>
                      <!--
                    <th style="white-space:nowrap;" class="text-center"><a href="#" role="button"  onclick="checkAll()" ><b>Check All</b></a></th>
    -->
                    <th style="white-space:nowrap;" class="text-center">No</th>
                    <th style="white-space:nowrap;" class="text-center">Code</th>
                    <th style="white-space:nowrap;" class="text-center">Name</th>
                    <th style="white-space:nowrap;" class="text-center">Company</th>
                  <th style="white-space:nowrap;" class="text-center">Company Structure</th>
                  <th style="white-space:nowrap;" class="text-center">Designation</th>
                  <th style="white-space:nowrap;" class="text-center">Period Date</th>
            ...
            foreach ($data_epms as $row) { ?>
             <tr>
              <!--
              <th align=center><input type="checkbox" name="pf[]" value="<?= $row['form_id'] ?>"></td>
            -->
                ...
                <td style="white-space:nowrap;" class="text-center">Cladtek</td>
          ...
          <td onclick="javascript: return confirm('Anda yakin ingin menghapus?')" style="white-space:nowrap;" class="text-center"><a href="<?= base_url() ?>home/deleteReport/<?= $row['form_id'] ?>" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a></td>
                            </tr>
                        <?php } ?>
                    <?php endif; ?>
                </tbody>
...
<script>
	function checkAll()
	{
		var box = document.getElementsByName("pf[]");
		box.forEach(
			function(index)
			{
  ...
  if (result.value) {
    ...
    form.trigger('submit');
    $.ajax({
      url : '<?= base_url() ?>gatepass/CheckDelete',
					type: 'DELETE',
			
    success:function(data){
      Swal.fire('Deleted!','Your file has been deleted','success')
      setTimeout(() => {
							location.href = '<?= base_url() ?>gatepass/index';
						}, 1000);
      $("#"+id).remove();
    },
    ...
    // },
    error:function(){
      Swal.fire('Error','Data failed to be deleted','error');
    }
    });
  }
});
